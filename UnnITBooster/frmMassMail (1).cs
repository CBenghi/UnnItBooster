...
namespace StudentMarking
{
    ...
    public partial class frmMassMail : Form
    {
        ...
        private void ReloadDB()
        {
            ...
            con.Open();
            var tables = con.GetOleDbSchemaTable(OleDbSchemaGuid.Tables, new object[] { null, null, null, "TABLE" });
            if (tables != null)
            {
                foreach (DataRow dataRow in tables.Rows)
                {
                    var nm = dataRow["TABLE_NAME"].ToString();
                    var tp = dataRow["TABLE_TYPE"].ToString();
                    if (tp == "TABLE")
                    {
                        cmbAvailableTables.Items.Add(nm);
                    }
                }
        ...
        private void ReloadTableData(OleDbConnection con, ref DataTable dt)
        {
            if (cmbAvailableTables.Items.Count <= 0) 
                return;
            var sql = string.Format("select * from [{0}]", cmbAvailableTables.SelectedItem);
